...
def ver_emergencias_para_taller(db: Session = Depends(get_db)):
    # Traer TODAS las emergencias que NO estén resueltas
    emergencias = db.query(models.Emergencia).filter(
        models.Emergencia.estado != "Resuelto"
    ).all()
    
    print(f"DEBUG: Emergencias no resueltas: {len(emergencias)}")
    for e in emergencias:
        print(f"  - ID: {e.id}, estado: '{e.estado}', cliente_id: {e.cliente_id}")
    
    # Devolver lista directa
    return emergencias
...
def ver_emergencias_de_cliente(cliente_id: int, db: Session = Depends(get_db)):
    ...
    for emergencia in emergencias:
        ...
        # Si tiene taller asignado, agregar datos del taller, distancia y tiempo
        if emergencia.taller_id and emergencia.estado in ["Aceptada", "En Camino", "Resuelto"]:
            taller = db.query(models.Taller).filter(models.Taller.id == emergencia.taller_id).first()
            if taller:
                # Obtener tiempo real de aceptaciones_taller
                aceptacion = db.query(models.AceptacionTaller).filter(
                    models.AceptacionTaller.emergencia_id == emergencia.id,
                    models.AceptacionTaller.taller_id == emergencia.taller_id
                ).first()
                tiempo_minutos = aceptacion.tiempo_estimado_minutos if aceptacion else 15
                item.update({
                    "nombre_taller": taller.nombre_taller,
                    "tiempo_estimado": f"{tiempo_minutos} mins",
                    "distancia_km": 3.5,
                })
...
def aceptar_emergencia(id_emergencia: int, taller_id: int = None, db: Session = Depends(get_db)):
    ...
    if emergencia:
        ...
        if taller_id:
            ...
            # Registrar la aceptación en la tabla historial
            nueva_aceptacion = models.AceptacionTaller(
                emergencia_id=id_emergencia,
                taller_id=taller_id,
                tiempo_estimado_minutos=15,  # Valor por defecto
                estado="Aceptada"
            )
...
# Ruta para que el taller marque como "Resuelto" (CU-09: Finalizar emergencia)
@app.patch("/emergencias/{id_emergencia}/finalizar")
def finalizar_emergencia(id_emergencia: int, db: Session = Depends(get_db)):
    emergencia = db.query(models.Emergencia).filter(models.Emergencia.id == id_emergencia).first()
    if emergencia:
        emergencia.estado = "Resuelto"
        db.commit()
        return {"mensaje": "Emergencia finalizada con éxito"}
    return {"error": "Emergencia no encontrada"}
# ---------------------------------------------------------
# CU-07: Visualizar Taller Asignado y ETA (DATOS REALES)
# ---------------------------------------------------------
...
async def obtener_taller_asignado(solicitud_id: int, db: Session = Depends(get_db)):
    # Buscar la emergencia en la base de datos
    emergencia = db.query(models.Emergencia).filter(models.Emergencia.id == solicitud_id).first()
    
    if not emergencia:
        return {"error": "Emergencia no encontrada", "estado": "No encontrada"}
    
    # Preparar respuesta base
    respuesta = {
        "id_solicitud": emergencia.id,
        "estado": emergencia.estado,
        "tipo_incidente": emergencia.tipo_ia,
        "severidad": emergencia.severidad_ia,
        "direccion": emergencia.direccion,
        "descripcion": emergencia.descripcion,
        "latitud": emergencia.latitud,
        "longitud": emergencia.longitud,
    ...
    # Si el taller ya aceptó, traer datos del taller
    if emergencia.taller_id and emergencia.estado in ["Aceptada", "En Camino", "Resuelto"]:
        taller = db.query(models.Taller).filter(models.Taller.id == emergencia.taller_id).first()
        if taller:
            # Obtener tiempo estimado real de la tabla aceptaciones_taller
            aceptacion = db.query(models.AceptacionTaller).filter(
                models.AceptacionTaller.emergencia_id == emergencia.id,
                models.AceptacionTaller.taller_id == emergencia.taller_id
            ).first()
            ...
            respuesta.update({
                ...
                "tiempo_estimado": f"{tiempo_minutos} mins",
                "distancia_km": 3.5  # En el futuro: calcular con GPS
            })
